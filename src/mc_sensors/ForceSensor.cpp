...
ForceSensor::ForceSensor() : Sensor() {}
...
const sva::PTransformd ForceSensor::X_0_f(const mc_tvm::Robot & robot) const
{
  return parentToSensor() * robot.bodyPosW(parent_);
}
...
{
  return (X_fsmodel_fsactual() * X_parent_sensor_).inv();
}
...
const sva::ForceVecd ForceSensor::wrenchWithoutGravity(const mc_tvm::Robot & robot) const
{
  sva::PTransformd X_0_p = robot.bodyPosW(parent_);
  auto w = wrench_ - calibration_->wfToSensor(X_0_p, X_parent_sensor_);
  return w;
}

sva::ForceVecd ForceSensor::worldWrench(const mc_tvm::Robot & robot) const
{
  sva::ForceVecd w_fsactual = wrench();
  sva::PTransformd X_parent_0 = robot.bodyPosW(parent_).inv();
  sva::PTransformd X_fsactual_0 = X_parent_0 * X_fsactual_parent();
  return X_fsactual_0.dualMul(w_fsactual);
}

sva::ForceVecd ForceSensor::worldWrenchWithoutGravity(const mc_tvm::Robot & robot) const
{

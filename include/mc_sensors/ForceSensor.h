 ...
 */
struct MC_SENSORS_DLLAPI ForceSensor : Sensor
{
public:
  ...
  const sva::PTransformd X_0_f(const mc_tvm::Robot & robot) const;
   ...
   *
   */
  const sva::ForceVecd wrenchWithoutGravity(const mc_tvm::Robot & robot) const;
   ...
   *
   */
  sva::ForceVecd worldWrench(const mc_tvm::Robot & robot) const;
   ...
   *
   */
  sva::ForceVecd worldWrenchWithoutGravity(const mc_tvm::Robot & robot) const;
   ...
   *
   */
  void loadCalibrator(const std::string & calib_file, const Eigen::Vector3d & gravity = mc_rtc::constants::gravity);
